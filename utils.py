...
# Field Size　→　OneHotEncoderの後、Fieldの個数
FIELD_SIZES = [0] * 26
...
# http://qiita.com/supersaiakujin/items/464cc053418e9a37fa7b
def init_var_map(init_vars, init_path=None):
    ...
    for var_name, var_shape, init_method, dtype in init_vars:
        ...
        if init_method == 'zero':
            ...
            var_map[var_name] = tf.Variable(tf.random_normal(var_shape, mean=0.0, stddev=STDDEV, dtype=dtype), dtype=dtype)
        elif init_method == 'tnormal':
            # Tensorを正規分布かつ標準偏差の２倍までのランダムな値で初期化する
            var_map[var_name] = tf.Variable(tf.truncated_normal(var_shape, mean=0.0, stddev=STDDEV, dtype=dtype), dtype=dtype)
        elif init_method == 'uniform':
            # Tensorを一様分布なランダム値で初期化する
            var_map[var_name] = tf.Variable(tf.random_uniform(var_shape, minval=MINVAL, maxval=MAXVAL, dtype=dtype), dtype=dtype)
